...
--]]
local COMBUCTOR_BANK_TEXT;
if (GetLocale() == "zhCN") then
	COMBUCTOR_BANK_TEXT = "银行";
elseif (GetLocale() == "zhTW") then
	COMBUCTOR_BANK_TEXT = "銀行";
else
	COMBUCTOR_BANK_TEXT = "Bank";
end
...
--	f:SetHeight(settings.h or BASE_HEIGHT)
	...
	if (lastID == 1) then		
		...
		search:SetPoint("TOPRIGHT", name, "TOPRIGHT", -130, -44);

		local btn = CreateFrame("Button", "CombuctorViewBank", _G[name], "UIPanelButtonTemplate");
		btn:SetHeight(26);
		btn:SetWidth(80);
		btn:SetPoint("TOPLEFT", name, "TOPLEFT", 70, -40);
		btn:SetText(">> " .. COMBUCTOR_BANK_TEXT);	
		local playerName = UnitName("player");
		btn:SetScript("OnClick", function(self)		
			if(CombuctorFrame2:IsVisible()) then
				self:SetText(">> " .. COMBUCTOR_BANK_TEXT);			
				HideUIPanel(CombuctorFrame2);				
			else
				self:SetText("<< " .. COMBUCTOR_BANK_TEXT);			
				ShowUIPanel(CombuctorFrame2);
				f:SetPlayer(playerName);
			end
		end);	
		...
		_G[name .. "Reset"]:Hide();
		--_G[name .. "BagToggle"]:Hide();
		_G[name .. "BagToggle"]:ClearAllPoints();
		_G[name .. "BagToggle"]:SetPoint("TOPRIGHT", name, "TOPRIGHT", -96, -38);
	end
...
function InventoryFrame:OnTitleEnter(title)
	Duowan_ShowNewbieTooltip(title:GetText(), L.MoveTip, L.ResetPositionTip, L.ScrollToZoom);
end
...
function InventoryFrame:SetItemFrameSize(width)
	...
	if self:IsBank() then
		minCol,minRow,maxCol,maxRow= 12,8,25,25
	else
		minCol,minRow,maxCol,maxRow= 8,8,20,20
	end
